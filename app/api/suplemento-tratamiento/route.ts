...
export async function GET(req: NextRequest) {
  ...
  const tratamientos = await prisma.suplementoTratamiento.findMany({
    where: { babyId },
...
export async function POST(req: NextRequest) {
  ...
  const t = await prisma.suplementoTratamiento.create({
    data: {
      babyId,
      nombre,
      dosis,
...
export async function PATCH(req: NextRequest) {
  ...
  const t = await prisma.suplementoTratamiento.update({
    where: { id },
    data: { activo },
    include: { administraciones: { orderBy: { administradoEn: "asc" } } },
  });
